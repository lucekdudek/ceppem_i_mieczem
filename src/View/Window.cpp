...
int Window::loadGLTexture(char *fileName) {
    ...
    if (!textureImage) {
        ...
        if (textureImage->format->BytesPerPixel == 4) {
            glTexImage2D(GL_TEXTURE_2D, 0, 4, textureImage->w, textureImage->h, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                         textureImage->pixels);
        } else {
            glTexImage2D(GL_TEXTURE_2D, 0, 3, textureImage->w, textureImage->h, 0, GL_RGB, GL_UNSIGNED_BYTE,
                         textureImage->pixels);
        }
...
int Window::renderText(char *text, TTF_Font *text_font) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, sdl_surface->w, sdl_surface->h, 0, GL_BGRA, GL_UNSIGNED_BYTE,
                 sdl_surface->pixels);
